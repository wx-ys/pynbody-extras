...
fn _rust(_py: Python<'_>, m: &PyModule) -> PyResult<()> {
    ...
    m.add_function(pyo3::wrap_pyfunction!(gravity::direct_accelerations_at_points_py, m)?)?;
    m.add_function(pyo3::wrap_pyfunction!(gravity::direct_potentials_at_points_py, m)?)?;
