...
def test_the_noise_family_is_reachable_from_the_image() -> None:
    ...
    gaussian = source.process.noise.gaussian(sigma=1.0, rng=0)
    poisson = source.process.noise.poisson(exposure=1.0, rng=0)
...
def test_the_noise_family_is_registered_and_exposed() -> None:
    ...
    assert hasattr(ImageData(np.zeros((4, 4))).process, "noise")
