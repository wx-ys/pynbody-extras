...
def test_smooth_gaussian_matches_the_free_function() -> None:
    ...
    via_method = source.smooth.gaussian(fwhm=2.0)
...
def test_smooth_methods_keep_the_geometry_and_metadata() -> None:
    ...
    for smoothed in (source.smooth.gaussian(fwhm=2.0), source.smooth.box(size=3), source.smooth.median(size=3)):
...
def test_smooth_methods_record_what_they_did() -> None:
    smoothed = image().smooth.gaussian(fwhm=2.0)
...
def test_box_and_median_methods_match_their_functions() -> None:
    ...
    np.testing.assert_allclose(source.smooth.box(size=3).data, box_smooth(source.data, 3))
    np.testing.assert_allclose(source.smooth.median(size=3).data, median_filter(source.data, 3))


def test_downsample_shrinks_the_image_and_its_edges() -> None:
    source = image((8, 8), extent=(0.0, 8.0, 0.0, 8.0))

    reduced = source.smooth.downsample(factor=2)
...
def test_kernel_widths_are_in_axis_units_on_a_uniform_grid() -> None:
    ...
    unit_based = source.smooth.gaussian(fwhm=4.0)
...
def test_psf_convolve_matches_the_free_function() -> None:
    ...
    via_method = source.psf.convolve(fwhm=2.0)
...
def test_psf_deconvolution_methods_return_images() -> None:
    ...
    blurred = image().psf.convolve(kernel)

    for restored in (
        blurred.psf.deconvolve(kernel),
        blurred.psf.wiener(kernel),
        blurred.psf.richardson_lucy(kernel, iterations=3),
    ):
...
def test_adaptive_bin_returns_a_map_carrying_this_image_geometry() -> None:
    ...
    assert binned.image.smooth.box(size=3).shape == binned.image.shape
