...
def test_views_forward_the_image_itself() -> None:
    ...
    for view in (source.process.smooth, source.process.psf, source.process.compose, source.process.adaptive):
...
def test_views_are_frozen() -> None:
    ...
    with pytest.raises(FrozenInstanceError):
        source.process.smooth.image = image()  # type: ignore[misc]
...
def test_every_capability_is_a_view_sharing_the_parent() -> None:
    ...
    views = (source.process.smooth, source.process.psf, source.process.compose, source.process.adaptive)
...
def test_views_expose_the_helpers_a_plugin_needs() -> None:
    ...
    assert uneven.process.smooth.kernel_scale() is None
...
def test_adaptive_map_is_a_view_of_its_painted_image() -> None:
    ...
    binned = source.process.adaptive.bin(signal, target_nbins=4)
...
def test_the_two_entry_points_are_registered_for_discovery() -> None:
    assert {"display", "process"} <= set(OPERATIONS)
